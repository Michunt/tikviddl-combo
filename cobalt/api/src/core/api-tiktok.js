...
// Override enabled services to only include TikTok
env.enabledServices = new Set(["tiktok"]);
...
export const runAPI = async (express, app, __dirname, isPrimary = true) => {
    ...
    const getServerInfo = () => {
        return JSON.stringify({
            cobalt: {
                ...
                services: ["TikTok"], // Only TikTok supported
            },
            git,
        });
    }
    ...
    // New standardized download endpoint
    app.post('/download', async (req, res) => {
        ...
        try {
            // Import TikTok service (use stub for local development if configured)
            const useStub = process.env.TIKTOK_STUB_MODE === 'true' || env.tiktokStubMode;
            const servicePath = useStub ? "../processing/services/tiktok-stub.js" : "../processing/services/tiktok.js";
            const tiktok = await import(servicePath);
            
            const result = await tiktok.default({
                postId: parsed.patternMatch.postId,
                shortLink: parsed.patternMatch.shortLink,
                fullAudio: normalizedRequest.tiktokFullAudio,
                isAudioOnly: normalizedRequest.isAudioOnly,
                h265: normalizedRequest.allowH265,
                alwaysProxy: normalizedRequest.alwaysProxy,
            });
            ...
            if (normalizedRequest.isAudioOnly) {
                const streamInfo = {
                    type: 'audio',
                    urls: [result.urls],
                    audioFormat: normalizedRequest.audioFormat,
            ...
            if (normalizedRequest.downloadMode === 'hd') {
                const streamInfo = {
                    type: 'hdEnhance',
                    urls: result.urls,
            ...
            const standardizedResponse = {
                auto: result.auto || (result.urls ? { url: result.urls, mime: 'video/mp4' } : null),
                audio: result.audio || (result.urls ? { url: result.urls, mime: 'audio/mpeg' } : null),
                hd: result.hd || (result.urls ? { url: result.urls, mime: 'video/mp4' } : null)
            };

            res.status(200).json({
                status: "stream",
                url: standardizedResponse.auto?.url || result.urls,
                formats: standardizedResponse
            });
        } catch (error) {
    ...
    app.get('/tunnel', apiTunnelLimiter, async (req, res) => {
        ...
        if (!streamInfo?.service) {
            return res.status(streamInfo?.status || 500).end();
        }
    ...
    http.createServer(app).listen({
        ...
        if (isPrimary) {
            console.log(`\n` +
                Bright(Cyan("TikViddl ")) + Bright("API ^ω^") + "\n" +
